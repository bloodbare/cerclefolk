...
base source: https://raw.github.com/quandyfactory/dicttoxml/master/dicttoxml.py
...
def convert_list(items, ids, parent):
    ...
    for i, item in enumerate(items):
        ...
        if type(item) in (int, float, str, unicode):
            ...
            addline('<event type="dict">%s</event>' % (convert_dict(item, ids, parent)))
        elif type(item) in (list, set, tuple) or isinstance(item, collections.Iterable):
            addline('<event type="list"%s>%s</event>' % (make_attrstring(attr), convert_list(item, ids, 'item')))
        else:
...
def dicttoxml(obj, root=True, ids=False):
    ...
    if root is True:
        ...
        addline('<apuntador>%s</apuntador>' % (convert(obj, ids, parent='root')))
    else:
